...
std::string iso8601() {
    ...
    size_t tsizTime = strftime(bufferTime, 26, "%Y-%m-%dT%H:%M:%S", &tmNow);    //Puts time into a string
    size_t tsizOffset = strftime(bufferTimezoneOffset, 6, "%z", &tmNow);        //Puts offset into a string
    std::string timeStamp = std::string() + bufferTime + bufferTimezoneOffset;  //Adds time and timezoneoffset together
    timeStamp.insert(22, ":");                                                  //Inserts ":" between hour and minutes of timezone offset
    return timeStamp;
}
...
void CreateAndSend10LastJSON() {
    ...
    for (int i = 0; i < 10; i++){
        ...
        web::json::keep_object_element_order(true);
        tempReading[L"time"][L"start"] = web::json::value::string(utility::conversions::to_string_t(timeBefore[orderOfReadings.at(i)])); //wont read string from vector
        tempReading[L"time"][L"end"] = web::json::value::string(utility::conversions::to_string_t(timeAfter[orderOfReadings.at(i)]));
        tempReading[L"min"] = web::json::value::number((floor(minTempValues[orderOfReadings.at(i)] / scale + 0.5) * scale));
        tempReading[L"max"] = web::json::value::number((floor(minTempValues[orderOfReadings.at(i)] / scale + 0.5) * scale));
        tempReading[L"average"] = web::json::value::number((floor(minTempValues[orderOfReadings.at(i)] / scale + 0.5) * scale));
    ...
    client.request(web::http::methods::POST, L"", TemperatureMeasurement.serialize(),
           L"application/json").then([](const web::http::http_response& response)
            {
...
void CreateAndSendSingleJSON() {
    ...
    */
    ...
    client.request(web::http::methods::POST, L"", TemperatureMeasurement.serialize().c_str(),   //Sends JSON
           L"application/json").then([](const web::http::http_response& response)  
            {
            std::wcout << response.status_code() << std::endl;
